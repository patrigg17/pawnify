...
function getAIMove(fen, callback) {
  if (!stockfish || !stockfish.ready) {
    // Fallback: random legal move, returned in UCI format for client compatibility
    const Chess = require('chess.js').Chess;
    const c = new Chess(fen);
    const moves = c.moves();
    const san = moves[Math.floor(Math.random() * moves.length)];
    console.log(`[getAIMove] Stockfish not ready, random SAN: ${san}`);
    // Convert SAN → UCI so client parsing (slice 0,2 / slice 2,4) works correctly
    const verbose = c.move({ from: san.slice(0, 2), to: san.slice(2, 4), promotion: san[4] || 'q' });
    if (!verbose) {
      // second attempt: parse as standard algebraic in case format differs
      const tryMove = c.move(san);
      if (!tryMove) return callback(new Error('Fallback move failed: ' + san));
      return callback(null, tryMove.from + tryMove.to + (tryMove.promotion ? tryMove.promotion : ''));
    }
    const uci = verbose.from + verbose.to + (verbose.promotion ? verbose.promotion : '');
    console.log(`[getAIMove] Random move converted to UCI: ${uci} (SAN was: ${san})`);
    return callback(null, uci);
  }
  console.log(`[getAIMove] Stockfish computing best move for FEN: ${fen}`);
  stockfish.getBestMove(fen, 18, (err, bestMove, ponder) => {
    console.log(`[getAIMove] Stockfish result: bestMove=${bestMove} ponder=${ponder} err=${err}`);
    callback(err, bestMove);
  });
}
...
app.get('/api/games/:gameId/ai-move', authMiddleware, (req, res) => {
  const game = db.prepare('SELECT * FROM games WHERE game_id=?').get(req.params.gameId);
  if (!game) return res.status(404).json({ error: 'Game not found' });
  console.log(`[/ai-move] gameId=${req.params.gameId} current FEN=${game.fen}`);
